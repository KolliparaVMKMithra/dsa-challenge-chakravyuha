...
class SIHTeamMemberCreate(BaseModel):
    full_name: str = Field(..., min_length=2, max_length=100)
    college_email: str = Field(..., min_length=5, max_length=100)
    personal_email: str = Field(..., min_length=5, max_length=100)
    phone_number: str = Field(..., min_length=10, max_length=20)
    study_year: int = Field(..., ge=1, le=4)
    branch: str = Field(..., pattern="^(CSE|AIE|AIDS|CCE|AEP|ECE|EAC)$") # Standard branch validator
...
class SIHTeamRegistration(BaseModel):
